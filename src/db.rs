use crate::{JendaError, Task, Tasks};
use chrono::prelude::*;
...
impl Database {
    ...
    pub fn insert(&mut self, task: Task) -> Result<(), JendaError> {
        ...
        match self.conn.execute(
            "INSERT INTO task (id, name, completed, timestamp) VALUES (?1, ?2, ?3, ?4)",
            (
    ...
    pub fn query_id(&self, id: &Uuid) -> Result<Task, JendaError> {
        let mut stmt = self
            .conn
            .prepare("SELECT id, name, completed, timestamp FROM task WHERE id = ?1")
